...
module.exports = class AccessUtils {
   ...
   *
   * @param {String|Object} modelToCheck Model class to check.
   * @returns {Boolean} Returns true if the Model is a group model.
   */
  isGroupModel(modelToCheck) {
    return this.isModelOfNamedType(modelToCheck, this.options.groupModel)
  }
   ...
   * @returns {Boolean} Returns true if the Model is a group model.
   */
  isGroupAccessModel(modelToCheck) {
    return this.isModelOfNamedType(modelToCheck, this.options.groupAccessModel)
  }

  /**
   * Check if a model class is the configured user model.
   *
   * @param {String|Object} modelToCheck Model class to check.
   * @returns {Boolean} Returns true if the Model is a group model.
   */
  isUserModel(modelToCheck) {
    return this.isModelOfNamedType(modelToCheck, this.options.userModel)
  }

  /**
   * Check if a model class is the configured user model.
   *
   * @param {String|Object} modelClassToCheck Model class to check.
   * @param {String|Object} modelClassNameToMatch Name of model type to check against.
   * @returns {Boolean} Returns true if the Model is a group model.
   */
  isModelOfNamedType(modelClassToCheck, modelClassNameToMatch) {
    if (modelClassToCheck) {
      const modelToMatch = this.app.models[modelClassNameToMatch]

      return modelClassToCheck === modelToMatch ||
        modelClassToCheck.prototype instanceof modelToMatch ||
        modelClassToCheck === modelClassNameToMatch
    }
    return false
  }
    ...
    return new Promise(function(resolve, reject){
      if (req){
        ...
        if (groupKey in req.headers){
          // extract the group Id from the header
          groupId = req.headers[groupKey];
        } else {
          // extract the group Id from the query
          for (var property in req.query){
            const property_lowercase = (property + "").toLowerCase();
            if (property_lowercase === groupKey){
              groupId = req.query[property];
            }
          }
        }
        ...
        if (groupId){
          ...
          debug('HTTP Req ' + groupKey + ' property could not be found')
          resolve(undefined);
        }
      } else {
        debug('HTTP Req undefined')
